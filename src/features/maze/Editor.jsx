    ...
    <div style={{height: '100%', display: 'flex', flexDirection: 'column'}}>
      <div style={{background:'#f8f9fa', padding:'10px', borderBottom:'1px solid #ddd'}}>
        <div style={{display:'flex', justifyContent:'center', gap:'20px', marginBottom:'10px', alignItems:'center'}}>
            <div style={{display:'flex', alignItems:'center', gap:'5px', background:'white', padding:'5px', borderRadius:'4px', border:'1px solid #eee'}}>
                ...
                <input type="number" min="3" max="50" value={cols} onChange={(e) => handleResize(rows, parseInt(e.target.value)||3)} style={{width:'40px', textAlign:'center', border:'1px solid #ccc', borderRadius:'3px'}} />
                <span style={{color:'#ccc'}}>x</span>
                <label style={{fontSize:'0.8rem', fontWeight:'bold', color:'#7f8c8d'}}>H:</label>
                <input type="number" min="3" max="50" value={rows} onChange={(e) => handleResize(parseInt(e.target.value)||3, cols)} style={{width:'40px', textAlign:'center', border:'1px solid #ccc', borderRadius:'3px'}} />
            </div>
            <div style={{display:'flex', alignItems:'center', gap:'5px'}}>
                <label style={{fontSize:'0.8rem'}}>Dir:</label>
                <input type="range" min="0" max="3" step="1" value={startPos.dir} onChange={(e) => updateDirection(e.target.value)} style={{width:'50px'}} />
                <span style={{fontSize:'1.2rem', transform: `rotate(${visualRotation}deg)`, display:'inline-block'}}>🤖</span>
            </div>
        </div>
      ...
      <div style={{flex: 1, padding: '20px', overflow: 'hidden', display:'flex', justifyContent:'center', alignItems:'center', background:'#e0e0e0'}}>
        <div className="editor-grid" style={{
                ...
                aspectRatio: `${cols} / ${rows}`, width: '100%', height: '100%', maxWidth: '100%', maxHeight: '100%', margin: 'auto'
            }}>
            {grid.map((row, rIndex) => (
                row.map((cell, cIndex) => {
                    ...
                    return (
                        <div key={`${rIndex}-${cIndex}`} onClick={() => handleCellClick(rIndex, cIndex)}
                            style={{ 
                                width: '100%', height: '100%', display:'flex', justifyContent:'center', alignItems:'center', cursor:'pointer', 
                                background: cell === 4 ? '#2c3e50' : (cell === 2 ? '#2ecc71' : '#fff'), position: 'relative', overflow: 'hidden'
                            }}>
      ...
      <style>{`.editor-grid { container-type: size; }`}</style>
    </div>
  );
}